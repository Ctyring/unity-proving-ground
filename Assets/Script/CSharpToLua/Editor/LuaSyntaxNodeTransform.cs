...
namespace CSharpLua {
    public partial class LuaSyntaxNodeTransform : CSharpSyntaxVisitor<LuaSyntaxNode> {
        ...
        private XmlMetaProvider XmlMetaProvider {
            get { return generator_.XmlMetaProvider; }
        }
        ...
        private LuaCompilationUnitSyntax CurCompilationUnit {
            get { return compilationUnits_.Peek(); }
        }
        ...
        private INamedTypeSymbol CurTypeSymbol {
            get { return typeDeclarations_.Peek().TypeSymbol; }
        }

        internal TypeDeclarationInfo CurTypeDeclaration {
            get { return typeDeclarations_.Count > 0 ? typeDeclarations_.Peek() : null; }
        }

        private LuaFunctionExpressionSyntax CurFunction {
            get { return functions_.Peek(); }
        }

        private LuaFunctionExpressionSyntax CurFunctionOrNull {
            get { return functions_.Count > 0 ? functions_.Peek() : null; }
        }

        private MethodInfo CurMethodInfoOrNull {
            get { return methodInfos_.Count > 0 ? methodInfos_.Peek() : null; }
        }
        ...
        private LuaBlockSyntax CurBlock {
            get { return blocks_.Peek(); }
        }

        private LuaBlockSyntax CurBlockOrNull {
            get { return blocks_.Count > 0 ? blocks_.Peek() : null; }
        }
        ...
        private LuaIdentifierNameSyntax GetTempIdentifier() {
            ...
            string name = LuaSyntaxNode.TempIdentifiers.GetOrDefault(index) ??
                          $"__temp{index - LuaSyntaxNode.TempIdentifiers.Length}__";
            ++CurBlock.TempCount;
            return name;
        }
        ...
        public LuaCompilationUnitSyntax VisitCompilationUnit(CompilationUnitSyntax node, bool isSingleFile = false) {
            LuaCompilationUnitSyntax compilationUnit =
                new LuaCompilationUnitSyntax(node.SyntaxTree.FilePath, !isSingleFile);
        ...
        private LuaSpecialGenericType CheckSpecialGenericArgument(INamedTypeSymbol typeSymbol) {
            ...
            if (interfaceType != null) {
                bool isBaseImplementation = typeSymbol.BaseType != null &&
                                            typeSymbol.BaseType.AllInterfaces.Any(i => i.IsGenericIEnumerableType());
        ...
        private void CheckTypeDeclaration(INamedTypeSymbol typeSymbol, LuaTypeDeclarationSyntax typeDeclaration,
            ...
            if (typeSymbol.IsRecord) {
                ...
                if (recordBaseType != null) {
                    typeDeclaration.AddBaseTypes(recordBaseType.ArrayOf(GetRecordInterfaceTypeName(typeSymbol)), null,
                        null);
                }
        ...
        private void CheckRecordParameterCtor(INamedTypeSymbol typeSymbol, TypeDeclarationSyntax node,
            LuaTypeDeclarationSyntax typeDeclaration) {
            if (typeSymbol.IsRecord) {
        ...
        private void BuildRecordParameterCtor(INamedTypeSymbol typeSymbol, LuaTypeDeclarationSyntax typeDeclaration,
            ...
            function.AddStatements(parameterList.Parameters.Select(i =>
                LuaIdentifierNameSyntax.This.MemberAccess(i).Assignment(i).ToStatementSyntax()));
            typeDeclaration.AddCtor(function, false);
            ...
            foreach (var p in ctor.Parameters) {
                ...
                if (expression != null) {
                    typeDeclaration.AddField(parameterList.Parameters[index], expression,
                        p.Type.IsImmutable() && isLiteral, false, false, true, null, false, false);
                }
        ...
        internal void AcceptPartialType(PartialTypeDeclaration major,
            IEnumerable<PartialTypeDeclaration> typeDeclarations) {
            ...
            foreach (var typeDeclaration in typeDeclarations) {
                ...
                var document = GetDocumentationComment(typeDeclaration.Node);
        ...
        private void GetTypeDeclarationName(BaseTypeDeclarationSyntax typeDeclaration, out LuaIdentifierNameSyntax name,
        ...
        private void VisitEnumDeclaration(INamedTypeSymbol typeSymbol, EnumDeclarationSyntax node,
            LuaEnumDeclarationSyntax enumDeclaration) {
        ...
        public override LuaSyntaxNode VisitEnumDeclaration(EnumDeclarationSyntax node) {
            GetTypeDeclarationName(node, out var name, out var typeSymbol);
            LuaEnumDeclarationSyntax enumDeclaration =
                new LuaEnumDeclarationSyntax(typeSymbol.ToString(), name, CurCompilationUnit);
        ...
        private void VisitAsync(bool returnsVoid, LuaFunctionExpressionSyntax function) {
            var invokeExpression =
                LuaIdentifierNameSyntax.System.MemberAccess(LuaIdentifierNameSyntax.Async).Invocation();
            var wrapFunction = new LuaFunctionExpressionSyntax();
        ...
        private MethodDeclarationResult BuildMethodDeclaration(
            ...
            // 是否是main入口
            else if (symbol.CheckMainMethod()) {
                isPrivate = false;
        ...
        private bool IsCurTypeExportMetadataAll {
            get { return generator_.Setting.IsExportMetadata || CurType.IsExportMetadataAll; }
        }

        private bool IsCurTypeSerializable {
            get { return IsCurTypeExportMetadataAll || CurTypeSymbol.IsSerializable; }
        }

        private static bool IsExportMethodDeclaration(BaseMethodDeclarationSyntax node) {
            // Debug.Log("body: " + node.Body + " node.ExpressionBody: " + node.ExpressionBody);
            return (node.Body != null || node.ExpressionBody != null) &&
                   !node.HasCSharpLuaAttribute(LuaDocumentStatement.AttributeFlags.Ignore);
        }
        ...
        public override LuaSyntaxNode VisitMethodDeclaration(MethodDeclarationSyntax node) {
            Debug.Log("[VisitMethodDeclaration] " + node);
            if (IsExportMethodDeclaration(node)) {
                // Debug.Log("-- IsExportMethodDeclaration --");
                // 构建方法声明
                var result = BuildMethodDeclaration(node, node.AttributeLists, node.ParameterList,
                    node.TypeParameterList, node.Body, node.ExpressionBody);
            ...
            // Debug.Log("-- not IsExportMethodDeclaration --");
        ...
        private void AddFieldMetaData(IFieldSymbol symbol, LuaIdentifierNameSyntax fieldName,
            List<LuaExpressionSyntax> attributes) {
            var data = new LuaTableExpression { IsSingleLine = true };
                ...
                foreach (var variable in node.Declaration.Variables) {
                    ...
                    if (node.IsKind(SyntaxKind.EventFieldDeclaration)) {
                        ...
                        if (!IsEventField(eventSymbol)) {
                            ...
                            LuaExpressionSyntax valueExpression = GetFieldValueExpression(typeSymbol,
                                variable.Initializer?.Value, out bool valueIsLiteral, out var statements);
                            LuaExpressionSyntax typeExpression = null;
                            if (isStatic) {
                                typeExpression = GetTypeName(eventSymbol.ContainingType);
                            }

                            var (add, remove) = CurType.AddEvent(eventName, innerName, valueExpression,
                                isImmutable && valueIsLiteral, isStatic, eventSymbol.IsPrivate(), typeExpression,
                                statements);
                            if (attributes.Count > 0 || variableSymbol.HasMetadataAttribute()) {
                                AddPropertyOrEventMetaData(variableSymbol, new PropertyMethodResult(add),
                                    new PropertyMethodResult(remove), null, attributes);
                            }
        ...
        private LuaExpressionSyntax GetFieldValueExpression(ITypeSymbol typeSymbol, ExpressionSyntax expression,
            out bool valueIsLiteral, out List<LuaStatementSyntax> statements) {
        ...
        private void AddField(LuaIdentifierNameSyntax name, ITypeSymbol typeSymbol, ExpressionSyntax expression,
            ...
            bool isMoreThanUpValueStaticInit = false) {
            var valueExpression =
                GetFieldValueExpression(typeSymbol, expression, out bool valueIsLiteral, out var statements);
        ...
        private class PropertyMethodResult {
            ...
            public PropertyMethodResult(LuaPropertyOrEventIdentifierNameSyntax name,
        ...
        private void AddPropertyOrEventMetaData(ISymbol symbol, PropertyMethodResult get, PropertyMethodResult set,
            ...
            if (!symbol.IsAbstract) {
                ...
                if (!hasGet && !hasSet) {
                    ...
                    if (isField) {
                        ...
                        var valueExpression = GetFieldValueExpression(typeSymbol, node.Initializer?.Value,
                            out bool valueIsLiteral, out var statements);
                        LuaExpressionSyntax typeExpression = null;
                        if (isStatic) {
                        ...
                        var (getName, setName) = CurType.AddProperty(propertyName, innerName, valueExpression,
                            isImmutable && valueIsLiteral, isStatic, isPrivate, typeExpression, statements, isGetOnly);
            ...
            private bool CheckInsertLuaCodeTemplate(string commentContent, out LuaStatementSyntax statement) {
                ...
                if (index != -1) {
                    ...
                    if (commentContent[index] == openBracket) {
                        string closeToken = LuaSyntaxNode.Tokens.CloseBracket + new string(equals, count) +
                                            LuaSyntaxNode.Tokens.CloseBracket;
                        int begin = index + 1;
        ...
        private LuaStatementSyntax InternalVisitReturnStatement(LuaExpressionSyntax expression,
            ReturnStatementSyntax node = null) {
        ...
        private LuaExpressionSyntax BuildCommonAssignmentExpression(LuaExpressionSyntax left, LuaExpressionSyntax right,
            string operatorToken, ExpressionSyntax rightNode, ExpressionSyntax parent) {
            ...
            if (left is LuaPropertyAdapterExpressionSyntax propertyAdapter) {
                ...
                if (isPreventDebugConcatenation) {
                    leftExpression =
                        new LuaInvocationExpressionSyntax(LuaIdentifierNameSyntax.SystemToString, leftExpression);
                }
                ...
                if (expression == null) {
                    bool isRightParenthesized = rightNode is BinaryExpressionSyntax ||
                                                rightNode.IsKind(SyntaxKind.ConditionalExpression);
        ...
        private LuaExpressionSyntax BuildCommonAssignmentExpression(ExpressionSyntax leftNode,
            ExpressionSyntax rightNode, string operatorToken, ExpressionSyntax parent) {
        ...
        private LuaExpressionSyntax BuildDelegateBinaryExpression(LuaExpressionSyntax left, LuaExpressionSyntax right,
            bool isPlus) {
            if (IsPreventDebug) {
                var methodName =
                    isPlus ? LuaIdentifierNameSyntax.DelegateCombine : LuaIdentifierNameSyntax.DelegateRemove;
        ...
        private LuaExpressionSyntax BuildDelegateAssignmentExpression(LuaExpressionSyntax left,
            LuaExpressionSyntax right, bool isPlus) {
            if (left is LuaPropertyAdapterExpressionSyntax propertyAdapter) {
                if (propertyAdapter.IsProperty) {
                    propertyAdapter.ArgumentList.AddArgument(
                        BuildDelegateBinaryExpression(propertyAdapter.GetCloneOfGet(), right, isPlus));
                    return propertyAdapter;
                }
        ...
        private LuaExpressionSyntax BuildBinaryInvokeAssignmentExpression(LuaExpressionSyntax left,
            LuaExpressionSyntax right, LuaExpressionSyntax methodName) {
            if (left is LuaPropertyAdapterExpressionSyntax propertyAdapter) {
        ...
        private LuaExpressionSyntax BuildBinaryInvokeAssignmentExpression(ExpressionSyntax leftNode,
            ExpressionSyntax rightNode, LuaExpressionSyntax methodName) {
        ...
        private LuaExpressionSyntax BuildLuaSimpleAssignmentExpression(LuaExpressionSyntax left,
        ...
        private List<LuaExpressionSyntax> BuildNumberNullableIdentifiers(ref LuaExpressionSyntax left,
            ref LuaExpressionSyntax right, bool isLeftNullable, bool isRightNullable) {
        ...
        private static void CheckNumberNullableCompareExpression(string operatorToken,
        ...
        private LuaExpressionSyntax BuildNumberNullableExpression(LuaExpressionSyntax left, LuaExpressionSyntax right,
            string operatorToken, bool isLeftNullable, bool isRightNullable, LuaIdentifierNameSyntax method) {
        ...
        private LuaExpressionSyntax BuildNumberNullableAssignment(LuaExpressionSyntax left, LuaExpressionSyntax right,
            string operatorToken, bool isLeftNullable, bool isRightNullable, LuaIdentifierNameSyntax method) {
            if (left is LuaPropertyAdapterExpressionSyntax propertyAdapter) {
                Contract.Assert(propertyAdapter.IsProperty);
                propertyAdapter.ArgumentList.AddArgument(BuildNumberNullableExpression(propertyAdapter.GetCloneOfGet(),
                    right, operatorToken, isLeftNullable, isRightNullable, method));
                return propertyAdapter;
            }

            return left.Assignment(BuildNumberNullableExpression(left, right, operatorToken, isLeftNullable,
                isRightNullable, method));
        }

        private bool IsNullableType(ExpressionSyntax leftNode, ExpressionSyntax rightNode, out bool isLeftNullable,
            out bool isRightNullable) {
        ...
        private bool IsNullableType(ExpressionSyntax leftNode, ExpressionSyntax rightNode) =>
            IsNullableType(leftNode, rightNode, out _, out _);

        private bool IsNullableAssignmentExpression(ExpressionSyntax left, ExpressionSyntax right, string operatorToken,
            out LuaExpressionSyntax result, LuaIdentifierNameSyntax method = null) {
            if (IsNullableType(left, right, out bool isLeftNullable, out bool isRightNullable)) {
                ...
                result = BuildNumberNullableAssignment(leftExpression, rightExpression, operatorToken, isLeftNullable,
                    isRightNullable, method);
                return true;
            }
        ...
        private bool IsNumberNullableAssignmentExpression(INamedTypeSymbol containingType, ExpressionSyntax left,
            ExpressionSyntax right, string operatorToken, out LuaExpressionSyntax result,
            LuaIdentifierNameSyntax method = null) {
        ...
        private LuaExpressionSyntax BuildNumberAssignmentExpression(ExpressionSyntax node, ExpressionSyntax leftNode,
            ExpressionSyntax rightNode, SyntaxKind kind) {
                    ...
                    switch (kind) {
                        ...
                        case SyntaxKind.SubtractAssignmentExpression: {
                            ...
                            if (containingType.IsDelegateType() ||
                                symbol.MethodKind is MethodKind.EventAdd or MethodKind.EventRemove) {
                                var left = leftNode.AcceptExpression(this);
                                var right = rightNode.AcceptExpression(this);
                            ...
                            if (IsPreventDebug) {
                                if (IsNumberNullableAssignmentExpression(containingType, leftNode, rightNode,
                                        isPlus ? LuaSyntaxNode.Tokens.Plus : LuaSyntaxNode.Tokens.Sub,
                                        out var result)) {
                                    return result;
                                }
                            }
                        ...
                        case SyntaxKind.MultiplyAssignmentExpression: {
                            if (IsNumberNullableAssignmentExpression(containingType, leftNode, rightNode,
                                    LuaSyntaxNode.Tokens.Multiply, out var result)) {
                                return result;
                            }
                        ...
                        case SyntaxKind.DivideAssignmentExpression: {
                            if (IsPreventDebug && containingType.IsDoubleOrFloatType(false)) {
                                if (IsNullableAssignmentExpression(leftNode, rightNode, LuaSyntaxNode.Tokens.Div,
                                        out var result)) {
                                    return result;
                                }
                            }
                            ...
                            if (containingType.IsIntegerType(false)) {
                                if (IsNullableType(leftNode, rightNode)) {
                                    if (IsLuaClassic || IsPreventDebug) {
                                        bool success = IsNullableAssignmentExpression(leftNode, rightNode, null,
                                            out var result, LuaIdentifierNameSyntax.IntegerDiv);
                                        Contract.Assert(success);
                                        return result;
                                    }
                                }

                                return BuildBinaryInvokeAssignmentExpression(leftNode, rightNode,
                                    LuaIdentifierNameSyntax.IntegerDiv);
                            }
                        ...
                        case SyntaxKind.ModuloAssignmentExpression: {
                            if (containingType.IsNumberType(false)) {
                                if (IsLuaClassic) {
                                    var method = containingType.IsIntegerType(false)
                                        ? LuaIdentifierNameSyntax.Mod
                                        : LuaIdentifierNameSyntax.ModFloat;
                                    if (IsNullableAssignmentExpression(leftNode, rightNode, null, out var result,
                                            method)) {
                                        return result;
                                    }
                                    ...
                                    if (IsPreventDebug && IsNullableAssignmentExpression(leftNode, rightNode, null,
                                            out var result, LuaIdentifierNameSyntax.Mod)) {
                                        return result;
                                    }
                        ...
                        case SyntaxKind.RightShiftAssignmentExpression: {
                            if (containingType.IsIntegerType(false)) {
                                ...
                                if (IsLuaClassic) {
                                    var method = isLeftShift
                                        ? LuaIdentifierNameSyntax.ShiftLeft
                                        : LuaIdentifierNameSyntax.ShiftRight;
                                    if (IsNullableAssignmentExpression(leftNode, rightNode, null, out var result,
                                            method)) {
                                        return result;
                                    }
                                ...
                                else if (IsPreventDebug && IsNullableAssignmentExpression(leftNode, rightNode,
                                             isLeftShift
                                                 ? LuaSyntaxNode.Tokens.ShiftLeft
                                                 : LuaSyntaxNode.Tokens.ShiftRight, out var result)) {
                                    return result;
                                }
                            }
        ...
        private LuaExpressionSyntax BuildLuaAssignmentExpression(AssignmentExpressionSyntax node,
            ExpressionSyntax leftNode, ExpressionSyntax rightNode, SyntaxKind kind) {
            ...
            switch (kind) {
                case SyntaxKind.SimpleAssignmentExpression: {
                    ...
                    else if (leftNode.IsKind(SyntaxKind.IdentifierName) &&
                             left == LuaIdentifierNameSyntax.Placeholder) {
                ...
                case SyntaxKind.AndAssignmentExpression: {
                    resultExpression = BuildBitAssignmentExpression(leftNode, rightNode, LuaSyntaxNode.Tokens.And,
                        LuaIdentifierNameSyntax.BitAnd, node);
                    break;
                }
                case SyntaxKind.ExclusiveOrAssignmentExpression: {
                    resultExpression = BuildBitAssignmentExpression(leftNode, rightNode, LuaSyntaxNode.Tokens.NotEquals,
                        LuaIdentifierNameSyntax.BitXor, node);
                    break;
                }
                case SyntaxKind.OrAssignmentExpression: {
                    resultExpression = BuildBitAssignmentExpression(leftNode, rightNode, LuaSyntaxNode.Tokens.Or,
                        LuaIdentifierNameSyntax.BitOr, node);
                    break;
                }
                case SyntaxKind.CoalesceAssignmentExpression: {
                    ...
                    LuaIfStatementSyntax ifStatement =
                        new LuaIfStatementSyntax(checkNilExpression.EqualsEquals(LuaIdentifierNameSyntax.Nil));
        ...
        private LuaIdentifierNameSyntax BuildBoolXorOfNullAssignmentExpression(LuaExpressionSyntax left,
            LuaExpressionSyntax right, bool isLeftNullable, bool isRightNullable) {
        ...
        private LuaExpressionSyntax BuildBoolXorOfNullAssignmentExpression(ExpressionSyntax leftNode,
            ExpressionSyntax rightNode, bool isLeftNullable, bool isRightNullable) {
            var left = VisitExpression(leftNode);
            var right = VisitExpression(rightNode);
            ...
            if (right.IsNil()) {
                ...
                result = BuildBoolXorOfNullAssignmentExpression(propertyAdapter.GetCloneOfGet(), right, isLeftNullable,
                    isRightNullable);
            }
            else {
        ...
        private LuaExpressionSyntax BuildLeftNullableBoolLogicAssignmentExpression(ExpressionSyntax leftNode,
            ExpressionSyntax rightNode, string boolOperatorToken) {
            var left = VisitExpression(leftNode);
            var right = VisitExpression(rightNode);
            var temp = GetTempIdentifier();
        ...
        private LuaExpressionSyntax BuildBitAssignmentExpression(ExpressionSyntax leftNode, ExpressionSyntax rightNode,
            string boolOperatorToken, LuaIdentifierNameSyntax methodName, ExpressionSyntax parent) {
            if (semanticModel_.GetSymbolInfo(parent).Symbol is IMethodSymbol symbol) {
                ...
                if (containingType != null) {
                    if (containingType.IsBoolType(false)) {
                        switch (parent.Kind()) {
                            case SyntaxKind.ExclusiveOrAssignmentExpression: {
                                if (IsNullableType(leftNode, rightNode, out bool isLeftNullable,
                                        out bool isRightNullable)) {
                                    return BuildBoolXorOfNullAssignmentExpression(leftNode, rightNode, isLeftNullable,
                                        isRightNullable);
                                }
                            ...
                            case SyntaxKind.AndAssignmentExpression:
                            case SyntaxKind.OrAssignmentExpression: {
                                if (IsNullableType(leftNode, rightNode, out bool isLeftNullable, out _) &&
                                    isLeftNullable) {
                                    return BuildLeftNullableBoolLogicAssignmentExpression(leftNode, rightNode,
                                        boolOperatorToken);
                                }
                    ...
                    if (containingType.IsIntegerType(false) || containingType.TypeKind == TypeKind.Enum) {
                        ...
                        else if (IsPreventDebug &&
                                 IsNullableAssignmentExpression(leftNode, rightNode, null, out var result,
                                     methodName)) {
                            return result;
                        }
                    }
        ...
        private class RefOrOutArgument {
            ...
            public RefOrOutArgument(LuaExpressionSyntax expression, ArgumentSyntax argument) {
                ...
                IsDeclaration = (argument.Expression.IsKind(SyntaxKind.DeclarationExpression) && !IsSpecial) ||
                                expression == LuaIdentifierNameSyntax.Placeholder;
            }
        ...
        private LuaExpressionSyntax BuildInvokeRefOrOut(CSharpSyntaxNode node, LuaExpressionSyntax invocation,
                ...
                case SyntaxKind.ConstructorDeclaration: {
                    ...
                    if (!symbol.ReturnsVoid || node.IsKind(SyntaxKind.ObjectCreationExpression)) {
                        var temp = node.Parent.IsKind(SyntaxKind.ExpressionStatement)
                            ? LuaIdentifierNameSyntax.Placeholder
                            : GetTempIdentifier();
        ...
        private bool IsEnumToStringInvocationExpression(IMethodSymbol symbol, InvocationExpressionSyntax node,
            ...
            if (symbol.Name == "ToString") {
                if (symbol.ContainingType.SpecialType == SpecialType.System_Enum) {
                    ...
                    result = BuildEnumToStringExpression(enumTypeSymbol, false, target,
                    ...
                    var memberAccessExpression = (MemberAccessExpressionSyntax)node.Expression;
                    var target = memberAccessExpression.Expression.AcceptExpression(this);
                    result = BuildEnumToStringExpression(enumTypeSymbol, isNullable, target,
                        memberAccessExpression.Expression);
                    return true;
                }
            }
        ...
        private List<Func<LuaExpressionSyntax>> FillCodeTemplateInvocationArguments(IMethodSymbol symbol,
            ArgumentListSyntax argumentList, List<Func<LuaExpressionSyntax>> argumentExpressions) {
            ...
            if (symbol.Parameters.Length > argumentList.Arguments.Count) {
                argumentExpressions.AddRange(symbol.Parameters.Skip(argumentList.Arguments.Count)
                    .Where(i => !i.IsParams).Select(i => {
                        var func = new Func<LuaExpressionSyntax>(() =>
                            GetDefaultParameterValue(i, argumentList.Parent, true));
                        return func;
                    }));
            }
        ...
        private LuaExpressionSyntax CheckCodeTemplateInvocationExpression(IMethodSymbol symbol,
            InvocationExpressionSyntax node) {
            var kind = node.Expression.Kind();
            if (kind is SyntaxKind.SimpleMemberAccessExpression or SyntaxKind.MemberBindingExpression
        ...
        private List<LuaExpressionSyntax> BuildInvocationArguments(IMethodSymbol symbol,
            InvocationExpressionSyntax node, out List<RefOrOutArgument> refOrOutArguments) {
            ...
            if (symbol != null) {
                ...
                foreach (var argument in node.ArgumentList.Arguments) {
                    ...
                    FillInvocationArgument(arguments, argument, ImmutableArray<IParameterSymbol>.Empty,
                        refOrOutArguments);
                }
            }
        ...
        private LuaInvocationExpressionSyntax CheckInvocationExpression(IMethodSymbol symbol,
            LuaExpressionSyntax expression) {
                    ...
                    if (memberAccess.Name is LuaInternalMethodExpressionSyntax) {
                        ...
                        if (IsPreventDebug && symbol is { IsStatic: false }) {
                            ...
                            if (containingType.IsBasicType()) {
                                ...
                                invocation = typeName.MemberAccess(memberAccess.Name)
                                    .Invocation(memberAccess.Expression);
                            }
                            else if (containingType.SpecialType == SpecialType.System_Object ||
                                     containingType.IsBasicTypInterface()) {
                                var methodMemberAccess = LuaIdentifierNameSyntax.System.MemberAccess(
                                    new LuaCodeTemplateExpressionSyntax(symbol.ContainingType.Name, memberAccess.Name));
        ...
        private LuaInvocationExpressionSyntax BuildExtensionMethodInvocation(IMethodSymbol reducedFrom,
            LuaExpressionSyntax expression) {
        ...
        private LuaExpressionSyntax GetDefaultParameterValue(IParameterSymbol parameter, SyntaxNode node,
        ...
        private void CheckInvocationDefaultArguments(
            ...
            if (parameters.Length > arguments.Count) {
                ...
                foreach (IParameterSymbol parameter in optionalParameters) {
                    if (parameter.IsParams) {
                        ...
                        LuaExpressionSyntax defaultValue =
            ...
            for (int i = 0; i < arguments.Count; ++i) {
                if (arguments[i] == null) {
                    LuaExpressionSyntax defaultValue =
                        GetDefaultParameterValue(parameters[i], node, isCheckCallerAttribute);
        ...
        private void CheckInvocationDefaultArguments(ISymbol symbol, ImmutableArray<IParameterSymbol> parameters,
            List<LuaExpressionSyntax> arguments, BaseArgumentListSyntax node) {
        ...
        private void CheckPrevIsInvokeStatement(ExpressionSyntax node) {
            ...
            if (curBlock != null) {
                var statement =
                    curBlock.Statements.FindLast(i => i is not LuaBlankLinesStatement && i is not LuaCommentStatement);
                if (statement != null) {
        ...
        private LuaExpressionSyntax BuildMemberAccessTargetExpression(ExpressionSyntax targetExpression) {
            ...
            if ((kind is >= SyntaxKind.NumericLiteralExpression and <= SyntaxKind.NullLiteralExpression) ||
            ...
            switch (symbol.Kind) {
                case SymbolKind.Field: {
                    ...
                    if (XmlMetaProvider.IsFieldForceProperty(fieldSymbol)) {
                        var expression = node.Expression.AcceptExpression(this);
                        var propertyIdentifierName =
                            new LuaPropertyOrEventIdentifierNameSyntax(true, GetMemberName(symbol));
                        return new LuaPropertyAdapterExpressionSyntax(expression, propertyIdentifierName,
        ...
        private LuaExpressionSyntax InternalVisitMemberAccessExpression(ISymbol symbol,
            ...
            if (node.Expression.IsKind(SyntaxKind.ThisExpression)) {
                ...
                if (symbol.Kind == SymbolKind.Method) {
                    if (IsDelegateExpression((IMethodSymbol)symbol, node, nameExpression, LuaIdentifierNameSyntax.This,
                            out var delegateExpression)) {
                ...
                if (symbol.Kind is SymbolKind.Property or SymbolKind.Event) {
                    switch (nameExpression) {
                        ...
                        case LuaMemberAccessExpressionSyntax memberAccess
                            when memberAccess.Expression == LuaIdentifierNameSyntax.This:
                            return nameExpression;
                        default:
            ...
            if (symbol.IsStatic && node.Expression.IsKind(SyntaxKind.IdentifierName) &&
                CurTypeSymbol.IsContainsInternalSymbol(symbol)) {
            ...
            return symbol.Kind switch {
                ...
                SymbolKind.Method when IsDelegateExpression((IMethodSymbol)symbol, node, name, expression,
                        out var delegateExpression) =>
                    delegateExpression,
        ...
        private bool IsDelegateExpression(IMethodSymbol symbol, MemberAccessExpressionSyntax node,
            LuaExpressionSyntax name, LuaExpressionSyntax expression, out LuaExpressionSyntax delegateExpression) {
        ...
        public override LuaSyntaxNode VisitMemberAccessExpression(MemberAccessExpressionSyntax node) {
            ...
            if (symbol == null) {
                ...
                var expression = node.Expression.AcceptExpression(this);
                bool isObjectColon = node.Parent.IsKind(SyntaxKind.InvocationExpression) &&
                                     expressSymbol.Kind != SymbolKind.NamedType;
                    ...
                    if (node.Parent.IsKind(SyntaxKind.SimpleAssignmentExpression)) {
                        ...
                        if (assignmentExpression.Left == node) {
                            CurType.AddStaticReadOnlyAssignmentName(name,
                                generator_.IsMoreThanUpValueStaticCtorField(symbol));
                        }
                    }
        ...
        private LuaExpressionSyntax VisitPropertyOrEventIdentifierName(IdentifierNameSyntax node, ISymbol symbol,
            bool isProperty, out bool isField) {
            bool isReadOnly;
            if (isProperty) {
                var propertySymbol = (IPropertySymbol)symbol;
        ...
        private void CheckDelegateBind(IMethodSymbol symbol, CSharpSyntaxNode node,
            ref LuaExpressionSyntax nameExpression) {
            ...
            if (symbol.IsGenericMethod) {
                ...
                if (!originalDefinition.EQ(symbol)) {
                    ...
                    foreach (var typeArgument in targetMethodSymbol.ContainingType.TypeArguments) {
                        if (typeArgument.TypeKind == TypeKind.TypeParameter) {
                            int parameterIndex =
                                targetMethodSymbol.Parameters.IndexOf(i => i.Type.IsTypeParameterExists(typeArgument));
                            if (parameterIndex == -1) {
                                Contract.Assert(targetMethodSymbol.ReturnType != null &&
                                                targetMethodSymbol.ReturnType.IsTypeParameterExists(typeArgument));
                    ...
                    foreach (var originalTypeArgument in originalDefinition.TypeArguments) {
                        ...
                        int parameterIndex =
                            originalDefinition.Parameters.IndexOf(i =>
                                i.Type.IsTypeParameterExists(originalTypeArgument));
                        if (parameterIndex != -1) {
                            ...
                            if (originalDefinition.ReturnType != null &&
                                originalDefinition.ReturnType.IsTypeParameterExists(originalTypeArgument)) {
                                originalTypeParameters.Add(new TypeParameterPlaceholder {
                                    Symbol = originalTypeArgument,
                                    ParameterIndex = kReturnParameterIndex,
                                });
                    ...
                    if (GenericPlaceholder.IsEnable(placeholders)) {
                        if (placeholders.TrueForAll(i => !i.IsTypeParameter)) {
                            if (placeholders.Count <= 3) {
                                string bindMethodName =
                                    LuaIdentifierNameSyntax.DelegateBind.ValueText + placeholders.Count;
                                var invocationExpression =
                                    new LuaInvocationExpressionSyntax(bindMethodName, nameExpression);
                            ...
                            switch (placeholders.Count) {
                                case 2 when placeholders[0].TypeParameterIndex == 2 &&
                                            placeholders[1].TypeParameterIndex == 1: {
                        ...
                        var invocation = new LuaInvocationExpressionSyntax(LuaIdentifierNameSyntax.DelegateBind,
                            nameExpression, symbol.Parameters.Length.ToString());
        ...
        private LuaExpressionSyntax BuildDelegateNameExpression(IMethodSymbol symbol, LuaExpressionSyntax target,
            LuaExpressionSyntax name, CSharpSyntaxNode node) {
        ...
        private LuaExpressionSyntax BuildDelegateNameExpression(IMethodSymbol symbol, LuaExpressionSyntax name,
            CSharpSyntaxNode node) {
        ...
        private LuaExpressionSyntax GetMethodNameExpression(IMethodSymbol symbol, NameSyntax node,
            ...
            if (IsInternalNode(node)) {
                if (IsParentDelegateName(node)) {
                    return BuildDelegateNameExpression(symbol, LuaIdentifierNameSyntax.This.MemberAccess(methodName),
                        node);
                }
        ...
        public override LuaSyntaxNode VisitIdentifierName(IdentifierNameSyntax node) {
            ...
            switch (symbol.Kind) {
                ...
                case SymbolKind.Field: {
                    ...
                    if (codeTemplate != null) {
                        identifier = BuildCodeTemplateExpression(codeTemplate,
                            fieldSymbol.IsStatic ? null : LuaIdentifierNameSyntax.This);
                        break;
                    }
        ...
        private void FillInvocationArgument(List<LuaExpressionSyntax> arguments, ArgumentSyntax node,
        ...
        private List<LuaExpressionSyntax> BuildArgumentList(ISymbol symbol, ImmutableArray<IParameterSymbol> parameters,
            ...
            foreach (var variable in node.Variables) {
                ...
                if (variable.Initializer != null && variable.Initializer.Value.IsKind(SyntaxKind.RefExpression)) {
                    ...
                    if (value.Expression.IsKind(SyntaxKind.InvocationExpression)) {
                        ...
                        AddRefInvocationVariableMapping((InvocationExpressionSyntax)value.Expression,
                            invocationExpression, variable);
                    }
                    else {
                    ...
                    if (!isConst) {
                        ...
                        if (variableDeclarator.Initializer == null) {
                            ...
                            if (typeSymbol.IsCustomValueType() &&
                                IsValueTypeVariableDeclarationWithoutAssignment(variable)) {
                                var typeExpression = GetTypeName(typeSymbol);
                                variableDeclarator.Initializer =
                                    new LuaEqualsValueClauseSyntax(BuildDefaultValue(typeExpression));
                            }
                        }
        ...
        private LuaExpressionSyntax BuildSwitchLabelWhenClause(LuaExpressionSyntax expression,
        ...
        private LuaExpressionSyntax BuildDeclarationPattern(DeclarationPatternSyntax declarationPattern,
            LuaIdentifierNameSyntax left, ExpressionSyntax expressionType, WhenClauseSyntax whenClause) {
            ...
            if (isExpression == LuaIdentifierLiteralExpressionSyntax.True) {
                return whenClause != null
                    ? whenClause.AcceptExpression(this)
        ...
        public override LuaSyntaxNode VisitCasePatternSwitchLabel(CasePatternSwitchLabelSyntax node) {
            ...
            switch (node.Pattern.Kind()) {
                case SyntaxKind.DeclarationPattern: {
                    ...
                    return BuildDeclarationPattern(declarationPattern, left, switchStatement.Expression,
                        node.WhenClause);
                }
                ...
                case SyntaxKind.RecursivePattern: {
                    ...
                    var switchStatement = (SwitchStatementSyntax)FindParent(node, SyntaxKind.SwitchStatement);
                    var expression =
                        BuildRecursivePatternExpression(recursivePattern, left, null, switchStatement.Expression);
        ...
        private void FillSwitchPatternSyntax(ref LuaIfStatementSyntax ifStatement, LuaExpressionSyntax condition,
            WhenClauseSyntax whenClause, LuaIdentifierNameSyntax assignmentLeft, ExpressionSyntax assignmentRight) {
        ...
        private void CheckSwitchDeconstruct(ref LuaLocalVariablesSyntax deconstruct, LuaIdentifierNameSyntax identifier,
            ExpressionSyntax type, int count) {
            if (deconstruct == null) {
        ...
        public LuaExpressionSyntax BuildPropertyPatternNameExpression(LuaIdentifierNameSyntax governingIdentifier,
            IdentifierNameSyntax nameIdentifier) {
        ...
        private LuaExpressionSyntax BuildRecursivePatternExpression(RecursivePatternSyntax recursivePattern,
            LuaIdentifierNameSyntax governingIdentifier, LuaLocalVariablesSyntax deconstruct,
            ...
            foreach (var subpattern in subpatterns) {
                ...
                if (subpattern.NameColon != null) {
                    ...
                    CheckSwitchDeconstruct(ref deconstruct, governingIdentifier,
                        recursivePattern.Type ?? governingExpression, subpatterns.Count);
            ...
            if (recursivePattern.Type != null) {
                var isExpression =
                    BuildIsPatternExpression(governingExpression, recursivePattern.Type, governingIdentifier);
                if (isExpression != LuaIdentifierLiteralExpressionSyntax.True) {
            ...
            foreach (var arm in node.Arms) {
                switch (arm.Pattern.Kind()) {
                    ...
                    case SyntaxKind.RecursivePattern: {
                        ...
                        var condition = BuildRecursivePatternExpression(recursivePattern, governingIdentifier,
                            deconstruct, node.GoverningExpression);
                        FillSwitchPatternSyntax(ref ifStatement, condition, arm.WhenClause, result, arm.Expression);
                        break;
                    }
                    case SyntaxKind.DeclarationPattern: {
                        var declarationPattern = (DeclarationPatternSyntax)arm.Pattern;
                        var condition = BuildDeclarationPattern(declarationPattern, governingIdentifier,
                            node.GoverningExpression, arm.WhenClause);
                    ...
                    case SyntaxKind.VarPattern: {
                        ...
                        var parenthesizedVariable =
                            (ParenthesizedVariableDesignationSyntax)varPatternSyntax.Designation;
                        int variableIndex = 0;
                        foreach (var variable in parenthesizedVariable.Variables) {
                            if (!variable.IsKind(SyntaxKind.DiscardDesignation)) {
                                CheckSwitchDeconstruct(ref deconstruct, governingIdentifier, node.GoverningExpression,
                                    parenthesizedVariable.Variables.Count);
                    ...
                    case SyntaxKind.OrPattern: {
                        var condition =
                            BuildPatternExpression(governingIdentifier, arm.Pattern, node.GoverningExpression);
                        FillSwitchPatternSyntax(ref ifStatement, condition, arm.WhenClause, result, arm.Expression);
                        break;
                    }
        ...
        private LuaExpressionSyntax BuildEnumToStringExpression(ITypeSymbol typeInfo, bool isNullable,
            LuaExpressionSyntax original, ExpressionSyntax node) {
            if (original is LuaLiteralExpressionSyntax) {
                var symbol = semanticModel_.GetSymbolInfo(node).Symbol;
                return new LuaConstLiteralExpression(new LuaStringLiteralExpressionSyntax(symbol!.Name),
                    typeInfo.ToString());
            }
            ...
            if (IsPreventDebug || isNullable) {
                return LuaIdentifierNameSyntax.System.MemberAccess(LuaIdentifierNameSyntax.EnumToString)
                    .Invocation(original, typeName);
            }
        ...
        private LuaExpressionSyntax WrapStringConcatExpression(ExpressionSyntax expression,
            LuaExpressionSyntax format = null, LuaExpressionSyntax alignment = null) {
        ...
        private LuaInvocationExpressionSyntax FormatAlignment(LuaExpressionSyntax e, LuaExpressionSyntax format,
            LuaExpressionSyntax alignment) {
        ...
        private LuaExpressionSyntax BuildBoolXorOfNullExpression(BinaryExpressionSyntax node, bool isLeftNullable,
        ...
        private LuaExpressionSyntax BuildLeftNullableBoolLogicExpression(BinaryExpressionSyntax node,
        ...
        private LuaExpressionSyntax BuildBitExpression(BinaryExpressionSyntax node, string boolOperatorToken,
                    ...
                    if (containingType.IsIntegerType(false) || containingType.TypeKind == TypeKind.Enum) {
                        if (IsLuaClassic) {
                            ...
                            return BuildBinaryInvokeExpression(node, bitMethodName);
                                 ...
                                 IsNullableBinaryExpression(node, null, out var result, bitMethodName)) {
                            return result;
                        }
                    }
                }
        ...
        private bool IsNullableBinaryExpression(BinaryExpressionSyntax node, string operatorToken,
            out LuaExpressionSyntax result, LuaIdentifierNameSyntax method = null) {
            if (IsNullableType(node, out bool isLeftNullable, out bool isRightNullable)) {
                var left = node.Left.AcceptExpression(this);
                var right = node.Right.AcceptExpression(this);
                result = BuildNumberNullableExpression(left, right, operatorToken, isLeftNullable, isRightNullable,
                    method);
                return true;
            }
        ...
        private bool IsNumberNullableBinaryExpression(INamedTypeSymbol containingType, BinaryExpressionSyntax node,
            string operatorToken, out LuaExpressionSyntax result, LuaIdentifierNameSyntax method = null) {
            if (containingType.IsNumberType(false)) {
                    ...
                    if (IsPreventDebug) {
                        switch (kind) {
                            ...
                            case SyntaxKind.SubtractExpression: {
                                ...
                                if (containingType.IsDelegateType()) {
                                    return BuildBinaryInvokeExpression(node,
                                        isPlus
                                            ? LuaIdentifierNameSyntax.DelegateCombine
                                            : LuaIdentifierNameSyntax.DelegateRemove);
                                }

                                if (IsNumberNullableBinaryExpression(containingType, node,
                                        isPlus ? LuaSyntaxNode.Tokens.Plus : LuaSyntaxNode.Tokens.Sub,
                                        out var result)) {
                                    return result;
                                }
                            ...
                            case SyntaxKind.GreaterThanOrEqualExpression: {
                                ...
                                if (IsNumberNullableBinaryExpression(containingType, node, operatorToken,
                                        out var result)) {
                    ...
                    switch (kind) {
                        case SyntaxKind.DivideExpression: {
                            ...
                            if (containingType.IsIntegerType(false)) {
                                if (IsNullableType(node)) {
                                    if (IsLuaClassic || IsPreventDebug) {
                                        bool success = IsNullableBinaryExpression(node, null, out var result,
                                            LuaIdentifierNameSyntax.IntegerDiv);
                                        Contract.Assert(success);
                                        return result;
                                    }
                        ...
                        case SyntaxKind.ModuloExpression: {
                            if (containingType.IsNumberType(false)) {
                                if (IsLuaClassic) {
                                    ...
                                    if (IsPreventDebug && IsNullableBinaryExpression(node, null, out var result,
                                            LuaIdentifierNameSyntax.Mod)) {
                                        return result;
                                    }
                        ...
                        case SyntaxKind.RightShiftExpression: {
                            if (containingType.IsIntegerType(false)) {
                                ...
                                string operatorToken = isLeftShift
                                    ? LuaSyntaxNode.Tokens.ShiftLeft
                                    : LuaSyntaxNode.Tokens.ShiftRight;
                                if (IsLuaClassic) {
                                    var method = isLeftShift
                                        ? LuaIdentifierNameSyntax.ShiftLeft
                                        : LuaIdentifierNameSyntax.ShiftRight;
                                    ...
                                    return BuildBinaryInvokeExpression(node, method);
                                         ...
                                         IsNullableBinaryExpression(node, operatorToken, out var result)) {
                                    return result;
                                }
                            }
        ...
        public override LuaSyntaxNode VisitBinaryExpression(BinaryExpressionSyntax node) {
            ...
            switch (kind) {
                ...
                case SyntaxKind.BitwiseAndExpression: {
                    resultExpression =
                        BuildBitExpression(node, LuaSyntaxNode.Tokens.And, LuaIdentifierNameSyntax.BitAnd);
                    break;
                }
                case SyntaxKind.ExclusiveOrExpression: {
                    resultExpression = BuildBitExpression(node, LuaSyntaxNode.Tokens.NotEquals,
                        LuaIdentifierNameSyntax.BitXor);
                    break;
                }
        ...
        private bool IsNullableTypeUserDefinedOperator(BinaryExpressionSyntax node, IMethodSymbol methodSymbol,
            out LuaExpressionSyntax result) {
            if (IsNullableType(node, out bool isLeftNullable, out bool isRightNullable)) {
        ...
        private LuaExpressionSyntax BuildNullableExpressionIdentifier(ExpressionSyntax node,
            List<LuaExpressionSyntax> identifiers) {
            var expression = node.AcceptExpression(this);
            return BuildNullableExpressionIdentifier(expression, identifiers);
        }

        private LuaIdentifierNameSyntax BuildNullableExpressionIdentifier(LuaExpressionSyntax expression,
            List<LuaExpressionSyntax> identifiers) {
        ...
        private void CheckIncrementExpression(ExpressionSyntax operand, ref LuaExpressionSyntax expression,
            bool isAddOrAssignment) {
            ...
            if (!symbol.IsNumberType()) {
                ...
                var methodSymbol = method.FirstOrDefault(i =>
                    isAddOrAssignment ? i.ReturnType.IsIntegerType() : i.ReturnType.EQ(symbol));
        ...
        private LuaSyntaxNode BuildPrefixUnaryExpression(bool isSingleLine, string operatorToken,
            LuaExpressionSyntax operand, PrefixUnaryExpressionSyntax node, bool isLocalVar = false) {
            var left = operand;
        ...
        private LuaSyntaxNode BuildPropertyPrefixUnaryExpression(bool isSingleLine, string operatorToken,
            LuaPropertyAdapterExpressionSyntax get, LuaPropertyAdapterExpressionSyntax set,
        ...
        private LuaMemberAccessExpressionSyntax GetTempUnaryExpression(LuaMemberAccessExpressionSyntax memberAccess,
            out LuaLocalVariableDeclaratorSyntax localTemp) {
            var temp = GetTempIdentifier();
        ...
        private LuaPropertyAdapterExpressionSyntax GetTempPropertyUnaryExpression(
            LuaPropertyAdapterExpressionSyntax propertyAdapter, out LuaLocalVariableDeclaratorSyntax localTemp) {
            var temp = GetTempIdentifier();
            localTemp = new LuaLocalVariableDeclaratorSyntax(temp, propertyAdapter.Expression);
            var result =
                new LuaPropertyAdapterExpressionSyntax(temp, propertyAdapter.Name, propertyAdapter.IsObjectColon);
            result.ArgumentList.AddArguments(propertyAdapter.ArgumentList.Arguments);
            return result;
        }

        private bool IsNullablePrefixUnaryExpression(PrefixUnaryExpressionSyntax node, LuaExpressionSyntax operand,
            string operatorToken, out LuaExpressionSyntax result, LuaIdentifierNameSyntax method = null) {
        ...
        public override LuaSyntaxNode VisitPrefixUnaryExpression(PrefixUnaryExpressionSyntax node) {
            ...
            if (kind == SyntaxKind.IndexExpression) {
                ...
                return new LuaInvocationExpressionSyntax(LuaIdentifierNameSyntax.Index, expression,
                    LuaIdentifierNameSyntax.True);
            }
            ...
            switch (kind) {
                ...
                case SyntaxKind.PreDecrementExpression: {
                    ...
                    string operatorToken = kind == SyntaxKind.PreIncrementExpression
                    ...
                    switch (operand) {
                        ...
                        case LuaPropertyAdapterExpressionSyntax propertyAdapter
                            when propertyAdapter.Expression != null: {
                            var getAdapter = GetTempPropertyUnaryExpression(propertyAdapter, out var localTemp);
                            CurBlock.Statements.Add(localTemp);
                            return BuildPropertyPrefixUnaryExpression(isSingleLine, operatorToken, getAdapter,
                                getAdapter.GetClone(), node);
                        }
                        case LuaPropertyAdapterExpressionSyntax propertyAdapter:
                            return BuildPropertyPrefixUnaryExpression(isSingleLine, operatorToken, propertyAdapter,
                                propertyAdapter.GetClone(), node);
                    }
                ...
                case SyntaxKind.BitwiseNotExpression: {
                    if (IsLuaClassic) {
                        if (IsNullablePrefixUnaryExpression(node, operand, null, out var result,
                                LuaIdentifierNameSyntax.BitNot)) {
                            return result;
                        }
                        ...
                        return LuaIdentifierNameSyntax.BitNot.Invocation(operand);
                             ...
                             IsNullablePrefixUnaryExpression(node, operand, LuaSyntaxNode.Tokens.BitNot,
                                 out var result)) {
                        return result;
                    }
                ...
                case SyntaxKind.UnaryMinusExpression: {
                    ...
                    if (IsPreventDebug &&
                        IsNullablePrefixUnaryExpression(node, operand, LuaSyntaxNode.Tokens.Sub, out var result)) {
                        return result;
                    }
                ...
                case SyntaxKind.LogicalNotExpression: {
                    ...
                    if (symbol != null && symbol.IsNullableType() && symbol.IsBoolType()) {
                        ...
                        ifStatement.Body.AddStatement(
                            temp.Assignment(new LuaPrefixUnaryExpressionSyntax(identifier, LuaSyntaxNode.Tokens.Not)));
                        CurBlock.AddStatement(ifStatement);
                        return temp;
                    }
        ...
        private LuaSyntaxNode BuildPostfixUnaryExpression(bool isSingleLine, string operatorToken,
            LuaExpressionSyntax operand, PostfixUnaryExpressionSyntax node) {
            if (isSingleLine) {
                var left = operand;
                CheckIncrementExpression(node.Operand, ref left, true);
                LuaExpressionSyntax binary = left.Binary(operatorToken, LuaIdentifierNameSyntax.One);
        ...
        private LuaSyntaxNode BuildPropertyPostfixUnaryExpression(bool isSingleLine, string operatorToken,
            LuaPropertyAdapterExpressionSyntax get, LuaPropertyAdapterExpressionSyntax set,
        ...
        public override LuaSyntaxNode VisitPostfixUnaryExpression(PostfixUnaryExpressionSyntax node) {
            ...
            bool isSingleLine = IsSingleLineUnary(node);
            string operatorToken = kind == SyntaxKind.PostIncrementExpression
                ? LuaSyntaxNode.Tokens.Plus
                : LuaSyntaxNode.Tokens.Sub;
            switch (operand) {
                case LuaMemberAccessExpressionSyntax memberAccess: {
                ...
                case LuaPropertyAdapterExpressionSyntax propertyAdapter when propertyAdapter.Expression != null: {
                    var getAdapter = GetTempPropertyUnaryExpression(propertyAdapter, out var localTemp);
                    CurBlock.Statements.Add(localTemp);
                    return BuildPropertyPostfixUnaryExpression(isSingleLine, operatorToken, getAdapter,
                        getAdapter.GetClone(), node);
                }
                case LuaPropertyAdapterExpressionSyntax propertyAdapter:
                    return BuildPropertyPostfixUnaryExpression(isSingleLine, operatorToken, propertyAdapter,
                        propertyAdapter.GetClone(), node);
                default:
        ...
        private void VisitLoopBody(StatementSyntax bodyStatement, LuaBlockSyntax block) {
            ...
            if (hasContinue) {
                ...
                LuaIfStatementSyntax ifStatement =
                    new LuaIfStatementSyntax(
                        new LuaPrefixUnaryExpressionSyntax(continueIdentifier, LuaSyntaxNode.Tokens.Not));
        ...
        private void CheckForeachCast(LuaIdentifierNameSyntax identifier, ForEachStatementSyntax node,
            LuaForInStatementSyntax forInStatement, bool isAsync) {
            var sourceType = semanticModel_.GetTypeInfo(node.Expression).Type;
            var targetType = semanticModel_.GetTypeInfo(node.Type).Type;
            bool hasCast = false;
            var elementType = !isAsync
                ? sourceType.GetIEnumerableElementType()
                : sourceType.GetIAsyncEnumerableElementType();
            if (elementType != null) {
            ...
            if (hasCast) {
                var cast = new LuaInvocationExpressionSyntax(LuaIdentifierNameSyntax.Cast, GetTypeName(targetType),
                    identifier);
        ...
        private LuaWhileStatementSyntax BuildWhileStatement(ExpressionSyntax nodeCondition,
            StatementSyntax nodeStatement) {
            LuaBlockSyntax conditionBody = new LuaBlockSyntax();
            PushBlock(conditionBody);
            ...
            if (conditionBody.Statements.Count == 0) {
                ...
                LuaIfStatementSyntax ifStatement =
                    new LuaIfStatementSyntax(new LuaPrefixUnaryExpressionSyntax(condition, LuaSyntaxNode.Tokens.Not));
                ...
                for (int i = begin + 1; i < end; ++i) {
                    ...
                    if (statement is LuaLocalDeclarationStatementSyntax declaration) {
                        ...
                        if (localVariable.Declarator.Initializer != null) {
                            luaBlock.Statements[i] = new LuaAssignmentExpressionSyntax(
                                localVariable.Declarator.Identifier, localVariable.Declarator.Initializer.Value);
                        }
                        else {
        ...
        private LuaExpressionSyntax BuildEnumCastExpression(LuaExpressionSyntax expression, ITypeSymbol originalType,
            ...
            if (targetType.TypeKind == TypeKind.Enum) {
                ...
                if (originalType.TypeKind == TypeKind.Enum || originalType.IsCastIntegerType()) {
                    var originalIntegerType = originalType.TypeKind == TypeKind.Enum
                        ? ((INamedTypeSymbol)originalType).EnumUnderlyingType
                        : originalType;
        ...
        private LuaExpressionSyntax BuildNumberCastExpression(LuaExpressionSyntax expression, ITypeSymbol originalType,
            ITypeSymbol targetType) {
            if (targetType.IsCastIntegerType()) {
            ...
            else if (targetType.SpecialType == SpecialType.System_Single &&
                     originalType.SpecialType == SpecialType.System_Double) {
                return GetCastToNumberExpression(expression, targetType, true);
            }
        ...
        private LuaExpressionSyntax BuildEnumAndNumberCastExpression(LuaExpressionSyntax expression,
            ITypeSymbol originalType, ITypeSymbol targetType) {
            return BuildEnumCastExpression(expression, originalType, targetType) ??
                   BuildNumberCastExpression(expression, originalType, targetType);
        }

        private LuaExpressionSyntax BuildNullableCastExpression(LuaExpressionSyntax expression,
            ITypeSymbol originalType, ITypeSymbol targetType) {
            ...
            if (targetNullableElementType != null) {
                if (originalNullableElementType != null) {
                    ...
                    var castExpression = BuildEnumAndNumberCastExpression(identifier, originalNullableElementType,
        ...
        private LuaExpressionSyntax BuildCastExpression(ITypeSymbol type, LuaExpressionSyntax expression) {
            ...
            var invocation =
                new LuaInvocationExpressionSyntax(LuaIdentifierNameSyntax.Cast, typeExpression, expression);
            if (type.IsNullableType()) {
        ...
        private LuaExpressionSyntax GetEnumNoConstantToNumberExpression(LuaExpressionSyntax expression,
            ITypeSymbol targetType) {
            string methodName = "System.Convert.To" + targetType.Name;
            return new LuaInvocationExpressionSyntax(methodName, expression);
        }

        private LuaExpressionSyntax GetCastToNumberExpression(LuaExpressionSyntax expression, ITypeSymbol targetType,
            bool isFromFloat) {
            if (expression is LuaParenthesizedExpressionSyntax parenthesizedExpression) {
                expression = parenthesizedExpression.Expression;
            }

            string name = (isFromFloat ? "To" : "to") +
                          (targetType.SpecialType == SpecialType.System_Char ? "UInt16" : targetType.Name);
